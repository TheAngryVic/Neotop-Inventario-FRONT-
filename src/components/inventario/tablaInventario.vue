...
<template>
  ...
  >
    ...
    <template #body="props" >
      <q-tr :props="props" :class=" props.row.Cantidad >= props.row.Modelo.stock_minimo ? 'bg-green-7' : 'bg-red-6' ">
        <q-td
          key="Modelo"
          :props="props"
          class="text-white"
...
export default {
  ...
  setup(props) {
    const inventario = inject("inventarioArray"); //Array con las productos
    ...
    const paginationGet = async (page = 0, size = 5, filter, sortBy, desc) => {
      try {
        ...
        if (filter!=="") {
          ...
          response = await api.get("/productos/inventario", filtrado)
        } else {
          if (sortBy) {
            
            response = await api.get("/productos/inventario", ordenado)
          } else {
            
            response = await api.get("/productos/inventario", options);
          }
        }
        ...
        inventario.value.splice(0, count, ...rows);
